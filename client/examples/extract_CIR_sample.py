...
#   python3 extract_CIR_sample.py --database db_name --ru_id 1 --ue_id 1 --sample 0
#
# Add --s3_endpoint, --s3_access_key, --s3_secret_key, or --iceberg_uri when
# connecting to non-default storage/catalog services.
#
# This script reads CIR rows for one sample/RU/UE tuple from the Iceberg cirs
# table and packs them into antenna-indexed complex impulse response buffers.
# It writes the extracted data to a sample-cir-XXXXXX.dat pickle file.
...
def load_table(
    ...
    query = f"SELECT {select_cols} FROM parquet_scan([{files_str}]) where time_idx ={sample} and ru_id={ru_id} and ue_id={ue_id}"
    return con.execute(query).fetchdf()

def build_cir_buffers(
    cirs,
    con: DuckDBPyConnection,
    s3_bucket: str,
    namespace_path: str,
    ...
    for idx in range(len(cirs)):
        ...
        ru_id = row["ru_id"]
        ue_id = row["ue_id"]
        ru_ant_el = row["ru_ant_el"]  # dict with keys "1","2","3" -> hor, ver, pol
        ue_ant_el = row["ue_ant_el"]
        cir_re = row["cir_re"]
        cir_im = row["cir_im"]
        cir_delay = row["cir_delay"]
        ...
        buffer_ru_delay = delays[sample_idx].get(ru_id, {})

        buffer_ue = buffer_ru.get(
            ue_id,
            np.zeros(
                (max_paths, n_ue_h * n_ue_v * n_ue_p, n_ru_h * n_ru_v * n_ru_p),
                dtype=complex,
            ),
        )
        buffer_ue_delay = buffer_ru_delay.get(
            ue_id,
            np.zeros(
                (max_paths, n_ue_h * n_ue_v * n_ue_p, n_ru_h * n_ru_v * n_ru_p),
                dtype=float,
        ...
        for tau in range(min(len(cir_delay), max_paths)):
            buffer_ue[tau, ue_cm_idx, ru_cm_idx] = complex(cir_re[tau], cir_im[tau])
            buffer_ue_delay[tau, ue_cm_idx, ru_cm_idx] = cir_delay[tau]

        buffer_ru[ue_id] = buffer_ue
        buffer_ru_delay[ue_id] = buffer_ue_delay
        data[sample_idx][ru_id] = buffer_ru
        delays[sample_idx][ru_id] = buffer_ru_delay
...
def import_data(args):
    ...
    cirs = load_table(con, args.s3_bucket, catalog, args.database, table_name="cirs", sample=args.sample, ru_id=args.ru_id, ue_id=args.ue_id)
    data, delays = build_cir_buffers(cirs, con, args.s3_bucket, args.database)
    return data, delays


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract CIR samples from Iceberg catalog via DuckDB",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Extract one CIR sample for a specific RU/UE pair
  python extract_CIR_sample.py --database db_name --ru_id 1 --ue_id 1 --sample 0
    ...
    parser.add_argument("--s3_provider", type=str, default="minio", choices=["minio", "aws"], help="S3 provider.")
    ...
    parser.add_argument("--ru_id", type=int, dest="ru_id", help="Specifies the RU ID", required=True)
    parser.add_argument("--ue_id", type=int, dest="ue_id", help="Specifies the UE ID", required=True)
    parser.add_argument("--sample", type=int, dest="sample", help="Specifies the sample to extract", required=True)

    args = parser.parse_args()

    data, delays = import_data(args)

    if len(data.keys()) > 0:
        with open(f"sample-cir-{str(args.sample).zfill(6)}.dat", "wb") as ofile:
            pickle.dump((data,delays), ofile, protocol=pickle.HIGHEST_PROTOCOL)
    else:
        print(f"No data found in the {args.database}.cirs!")
