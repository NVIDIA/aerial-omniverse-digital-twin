...
#   python3 extract_CFR_sample.py --database basic6 --ru_id 1 --ue_id 1 --sample 0
#
# Add --s3_endpoint, --s3_access_key, --s3_secret_key, or --iceberg_uri when
# connecting to non-default storage/catalog services.
#
# This script reads CFR rows for one sample/RU/UE tuple from the Iceberg cfrs
# table and packs them into antenna-indexed complex frequency response buffers.
# It writes the extracted data to a sample-cfr-XXXXXX.dat pickle file.
...
def load_table(
    ...
    if sample is not None:
        filters.append(f"time_idx ={sample}")
    if ru_id is not None:
        filters.append(f"ru_id={ru_id}")
    if ue_id is not None:
        filters.append(f"ue_id={ue_id}")
    where_clause = f" where {' and '.join(filters)}" if filters else ""
    query = f"SELECT {select_cols} FROM parquet_scan([{files_str}]){where_clause}"
...
def build_cfrs_buffers(
    ...
    panels = con.execute(
        "SELECT panel_id, dual_polarized, num_loc_antenna_horz, num_loc_antenna_vert,frequencies "
        f"FROM parquet_scan(['s3://{s3_bucket}/{namespace_path}/panels/data/panels.parquet'])"
    ).fetchdf()

    rus=con.execute(
        f"SELECT fft_size,subcarrier_spacing FROM parquet_scan(['s3://{s3_bucket}/{namespace_path}/rus/data/rus.parquet'])"
    ).fetchdf()
    fft_size=rus.iloc[0]["fft_size"]
    scs=rus.iloc[0]["subcarrier_spacing"]
    ue_fc=panels.iloc[0]["frequencies"]
    ru_fc=panels.iloc[0]["frequencies"]
    ...
    return fft_size, scs, ue_fc,ru_fc,data
...
def import_data(args):
    ...
    cfrs = load_table(con, args.s3_bucket, catalog, args.database, table_name="cfrs", sample=args.sample, ru_id=args.ru_id, ue_id=args.ue_id)
    fft_size, scs, ue_fc,ru_fc,data = build_cfrs_buffers(cfrs, con, args.s3_bucket, args.database)
    return fft_size, scs, ue_fc,ru_fc,data
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Extract one CFRS sample for a specific RU/UE pair
  python extract_CFR_sample.py --database db_name --ru_id 1 --ue_id 1 --sample 0
        """
    )
    ...
    fft_size, scs, ue_fc,ru_fc,data = import_data(args)

    if len(data.keys()) > 0:
        with open(f"sample-cfr-{str(args.sample).zfill(6)}.dat", "wb") as ofile:
            pickle.dump((fft_size, scs, ue_fc,ru_fc, data), ofile, protocol=pickle.HIGHEST_PROTOCOL)
    else:
        print(f"No data found in the {args.database}.cfrs!")
